...
def test_map_signals_to_latent_expected_values():
    ...
    assert latent.risk_sensitivity == pytest.approx(0.535)
    assert latent.patience_level == pytest.approx(0.335)
    assert latent.analytical_thinking == pytest.approx(0.45)
    assert latent.controlled_perception == pytest.approx(0.51)
